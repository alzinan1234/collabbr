...
export default function VerifiedBadges() {
  const [userState, setUserState] = useState({
    ...
    autoRenew: true,
    // History array added for dynamic table
    history: [
      {
        id: "1",
  ...
  const handleAction = (tierId) => {
    ...
    const newTransaction = {
      ...
      expireDate: "05 Nov 2026, 11:45 PM", // Future date simulation
      amount: tier.price,
      status: tier.price > (activeBadge?.price || 0) ? "Upgrade" : "Purchase"
    };
    ...
    setUserState(prev => ({
      ...
      history: [newTransaction, ...prev.history] // Prepend new transaction
    }));
  };
      ...
      <div className="p-8 min-h-screen font-sans text-slate-800 max-w-[1440px] mx-auto pt-30">
              ...
              <>
                <div className="flex justify-between items-start">
                  <div className="flex gap-4">
                    <div className={`w-14 h-14 ${activeBadge?.bg} rounded-2xl flex items-center justify-center text-white shadow-lg`}>
                      {activeBadge && React.createElement(iconMap[activeBadge.icon], { size: 28 })}
                    </div>
                    <div>
                  ...
                  <span className={`px-4 py-1 rounded-full text-xs font-semibold ${
                    userState.status === 'active' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-500'
                  }`}>
                ...
                <div className="mt-6 flex justify-between items-end">
                  ...
                  {userState.status === 'active' ? (
                    ...
                    <button onClick={() => handleAction(userState.currentBadgeId)} className="bg-indigo-600 text-white px-8 py-2 rounded-lg font-semibold text-sm">Purchase</button>
                  )}
                </div>
              </>
            )}
                ...
                return (
                  <div key={tier.id} className="flex flex-col items-center gap-2">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-500 ${
                      isEarned ? `${tier.bg} text-white shadow-md` : 'bg-slate-100 text-slate-400 border border-dashed border-slate-200'
                    }`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-12">
          {BADGE_TIERS.map((tier) => {
            ...
            return (
              <div key={tier.id} className={`bg-white p-5 rounded-2xl border-2 transition-all ${isActive ? 'border-indigo-100 shadow-md ring-2 ring-indigo-50' : 'border-slate-50'}`}>
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-center">
                          <button className={`px-4 py-1.5 rounded-lg text-xs font-bold min-w-[85px] transition-all hover:opacity-80 ${
                            item.status === 'Upgrade' ? 'bg-green-50 text-green-600' : 'bg-indigo-50 text-indigo-600'
                          }`}>
                            {item.status}
                          </button>
                        </td>
